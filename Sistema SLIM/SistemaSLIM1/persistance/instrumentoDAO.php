...
class instrumentoDAO {
    ...
    function salvar($instrumento, $conexao) {
        $sql = "INSERT INTO instrumento (Tipo, Id, Nome,Preco, Descr) VALUES ('" .
        $instrumento->getTipo() ."','".
        $instrumento->getId() ."','".
        $instrumento->getNome() ."','".
    ...
    function consultarInstrumento($conexao) {
        $sql = "SELECT Tipo, Id, Nome, Preco, Descr FROM instrumento";
        $res = $conexao->query($sql);
        return $res;
    }
    function consultarInstrumentoID($Id ,$conexao) {
        $sql = "SELECT Tipo, Id, Nome, Preco, Descr FROM instrumento WHERE Id=".$Id;
        $res = $conexao->query($sql);
        return $res;
    }
