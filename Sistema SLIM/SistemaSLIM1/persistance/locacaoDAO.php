...
class locacaoDAO {
    ...
    function salvar($instrumento, $conexao) {
        $sql = "INSERT INTO locacao (Nome, InstrumentoID ,Dia ,Preco, Id) VALUES ('" .
        $instrumento->getNome() ."','".
    ...
    function consultarLocacao($conexao) {
        $sql = "SELECT Nome, InstrumentoID, Dia, Preco, Id FROM locacao";
        $res = $conexao->query($sql);
        return $res;
    }
    function consultarLocacaoID($Id ,$conexao) {
        $sql = "SELECT Nome, InstrumentoID, Dia, Preco, Id FROM locacao WHERE Id=".$Id;
        $res = $conexao->query($sql);
        return $res;
    }
    ...
    function alterarLocacaoID($ins ,$conexao){
        ...
        $sql= "UPDATE locacao SET Nome='$nome', InstrumentoID= $instrumentoID, Dia =$dia, Preco =$preco, Id= $id WHERE Id=$id";
        echo "$sql";
        $res= mysqli_query($conexao, $sql);
        return $res;
      }
